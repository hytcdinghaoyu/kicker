...
function FSMenu(myName,nested,cssProp,cssVis,cssHid){
	...
	this.animInSpeed=0.1;
	this.animOutSpeed=0.1;
	this.animations=[]
};
		...
		with(node)with(obj){
			...
			if(menuToShow.length){
				...
				if(c!=child&&c.lyr.ref){
					...
					//setTimeout('with('+myName+'){menus["'+c.id+'"].par=menus["'+node.id+'"];menus["'+c.id+'"].show()}',delay?delay:1);
				}
				menuToShow.length=0
			}
	...
	with(this)with(obj){
		...
		with(lyr){
			...
			if(isDOM&&(sh?counter<1:counter>0)){
				timer=setTimeout(myName+'.menus["'+id+'"].setVis('+sh+','+(forced||0)+')',0);
			}
			else{
				lyr.forced=false;
			}
...
FSMenu.animSwipeDown=function(ref,counter,show){
	return;
	var elm=ref.firstChild.style?ref.firstChild:ref.firstChild.nextSibling,isOldIE=/MSIE\s(5|6|7)\./.test(navigator.userAgent);
	if(!elm)return;
	if(show&&(counter==0)){
...
FSMenu.animFade=function(ref,counter,show){
	...
	var f=ref.filters,done=(show?counter==1:counter<0.01),a=/MSIE\s(4|5)/.test(navigator.userAgent)?'alpha':'DXImageTransform.Microsoft.Alpha';
	if(f){
...
FSMenu.animClipDown=function(ref,counter,show){
	return;
	var cP=Math.pow(Math.sin(Math.PI*counter/2),0.75);
	ref.style.clip=(counter==1?((window.opera||navigator.userAgent.indexOf('KHTML')>-1)?'':'rect(auto,auto,auto,auto)'):'rect(0,'+ref.offsetWidth+'px,'+(ref.offsetHeight*cP)+'px,0px)')
};
FSMenu.prototype.activateMenu=function(id,subInd){
	with(this){
		...
		i++){
			...
			var eShow=new Function('with('+myName+'){var m=menus["'+menuID+'"],pM=menus["'+parUL.id+'"];if(!showOnClick||(showOnClick==1&&!'+rootItem+')||((showOnClick<=2)&&((pM&&pM.child)||(m&&m.visible))))show("'+menuID+'",this)}');
			var eHide=new Function('e','if(e.'+evtProp+'!=false)'+myName+'.hide("'+menuID+'")');
			addEvent(a,'mouseover',eShow);
			addEvent(a,'mouseout',eHide);
			addEvent(a,'focus',eShow);
			addEvent(a,'blur',eHide);
			addEvent(a,'click',new Function('e','var s='+myName+'.showOnClick,m='+myName+'.menus["'+menuID+'"];if(!((s==1&&'+rootItem+')||s>=2))return;'+myName+'[m&&m.visible?"hide":"show"]("'+menuID+'",this);if(e.cancelable&&e.preventDefault)e.preventDefault();e.'+evtProp+'=false;return false'));
		...
		if(hideOnClick)addEvent(mRoot,'click',new Function('e','if(e.'+evtProp+'!=false)'+myName+'.hideAll()'));
		menus[id]=new FSMenuNode(id,true,this)
	}

};
var page={
	win:self,minW:0,minH:0,MS:isIE&&!isOp,db:document.compatMode&&document.compatMode.indexOf('CSS')>-1?'documentElement':'body'
};
